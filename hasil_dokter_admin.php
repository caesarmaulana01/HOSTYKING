    ...
    if (isset($_POST['update_status'])) {
        ...
        echo '<script>
                setTimeout(function() {
					...
					window.location.href = "hasil_dokter_admin.php?step=pasien_tindakan";
				});
                }, 500);
            </script>';
	}
						...
						<div class="row">
							<div class="col-12">
								<div class="card">
									...
									<?php if (empty($_POST) && (isset($_GET['step']) && $_GET['step'] == 'pasien_tindakan')) { ?>
									...
									<div class="card-body">
										<div class="table-responsive">
                                            <table class="table table-striped" id="table-1">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $sql = mysqli_query($conn, "SELECT DISTINCT p.id, p.nama_pasien, d.nama_dokter, rp.penyakit 
													FROM riwayat_tindakan rt 
													JOIN pasien p ON rt.id_pasien = p.id 
													JOIN data_dokter d ON rt.id_dokter = d.id 
													JOIN riwayat_penyakit rp ON rt.id_penyakit = rp.id 
                                                    ...
                                                    ?>
                                                        <tr>
															...
															<form method="POST">
																...
																<button class="btn btn-icon icon-right btn-primary" name="jalan2"><i class="fas fa-eye"></i></i></button>
                                                            ...
                                                            <div class="modal-dialog" role="document">
                                                                <div class="modal-content">
                                                                    ...
                                                                    <div class="modal-body">
                                                                        <p><strong>Detail:</strong> <?php echo $row['detail']; ?></p>
                                                                    ...
                                                                    <div class="modal-footer">
                                                                        <?php if ($row['status'] == 'pending') { ?>
                                                                            <form method="POST">
                                                                                ...
                                                                                <button type="submit" class="btn btn-primary" name="update_status">Tandai Selesai</button>
                                                                            </form>
                                                                        <?php } ?>
								...
								if (isset($_POST['jalan2'])) {
									...
									$sql_detail = mysqli_query($conn, "SELECT rt.id, rt.detail, rt.harga, rt.status 
                                       									FROM riwayat_tindakan rt 
                                       									WHERE rt.id_pasien = '$id_pasien' AND rt.status = 'pending'"); ?>											
								<div class="card-body">
                                    <div class="table-responsive">
										<table class="table table-striped" id="table-1">
											<thead>
											...
											<tbody>
												<?php
												$sql = mysqli_query($conn, "SELECT rt.id, p.nama_pasien, d.nama_dokter, rp.penyakit, rt.detail, rt.harga, rt.status 
												FROM riwayat_tindakan rt 
												JOIN pasien p ON rt.id_pasien = p.id 
												JOIN data_dokter d ON rt.id_dokter = d.id 
												JOIN riwayat_penyakit rp ON rt.id_penyakit = rp.id 
												WHERE rt.status = 'pending' AND rt.id_pasien='$id_pasien'");
									...
									<div class="btn-group">
											<a href="hasil_dokter_admin.php?step=pasien_tindakan" class="btn btn-secondary">
												Kembali
											</a>
										</div>
								</div>
