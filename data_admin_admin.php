...
<head>
	...
	if (isset($_POST['submit'])) {
		...
		if ($old_pass == "" && $new_pass == "") {
			$up1 = mysqli_query($conn, "UPDATE data_admin SET nama_admin='$nama', username='$user', alamat='$alam' WHERE id='$id'");
			echo '<script>
			setTimeout(function() {
				swal({
					title: "Data Diubah",
			...
			$cekpass = mysqli_query($conn, "SELECT * FROM data_admin WHERE id='$id' AND password='$old_pass'");
			...
			if ($cekada == 0) {
				...
				$up2 = mysqli_query($conn, "UPDATE data_admin SET nama_admin='$nama', username='$user', password='$new_pass', alamat='$alam', senin='$senin', selasa='$selasa', rabu='$rabu', kamis='$kamis', jumat='$jumat', sabtu='$sabtu' WHERE id='$id'");
				echo '<script>
				setTimeout(function() {
					swal({
					title: "Data Diubah",
	...
	if (isset($_POST['submit2'])) {
		...
		$cekuser = mysqli_query($conn, "SELECT * FROM data_admin WHERE username='$user'");
		...
		if ($baris >= 1) {
			...
			$add = mysqli_query($conn, "INSERT INTO data_admin (username, password, nama_admin, alamat, pekerjaan) VALUES ('$user', '$pass', '$nama', '$alam', '$job')");
			echo '<script>
				setTimeout(function() {
					swal({
						title: "Berhasil!",
												...
												<tbody>
													...
													?>
														<tr>
															...
															<td><?php if ($row['pekerjaan'] == '1') {
																	echo '<div class="badge badge-pill badge-primary mb-1">Admin';
																} else {
																	echo '<div class="badge badge-pill badge-success mb-1">Supervisor Admin';
																} ?>
										</div>
										</td>
										<td>
											<span data-target="#editUser" data-toggle="modal" data-id="<?php echo $row['id']; ?>" data-nama="<?php echo $row['nama_admin']; ?>" data-user="<?php echo $row['username']; ?>" data-alam="<?php echo $row['alamat']; ?>">
												<a class="btn btn-primary btn-action mr-1" title="Edit" data-toggle="tooltip"><i class="fas fa-pencil-alt"></i></a>
											</span>
											<a class="btn btn-danger btn-action" data-toggle="tooltip" title="Hapus" data-confirm="Hapus Data|Apakah anda ingin menghapus data ini?" data-confirm-yes="window.location.href = 'auth/delete_admin.php?type=data_admin&id=<?php echo $row['id']; ?>'" ;><i class="fas fa-trash"></i></a>
										</td>
										</tr>
									<?php } ?>
									</tbody>
						...
						<form action="" method="POST" class="needs-validation" novalidate="">
							...
							<div class="form-group">
								...
								<select class="form-control selectric" name="pekerjaan">
									...
									<option value="2">Supervisor Admin</option>
								</select>
							</div>
							<div class="form-group">
								<label>Alamat</label>
