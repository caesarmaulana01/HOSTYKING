											...
											<table class="table table-striped" id="table-1">
												...
												<?php
														...
														while ($row = mysqli_fetch_array($sql)) {
															...
															// Ambil id_penyakit dari tabel riwayat_penyakit berdasarkan id_pasien
															$id_penyakit = null;
															if (!empty($defpasien)) {
																$query_penyakit = mysqli_query($conn, "SELECT id FROM riwayat_penyakit WHERE id_pasien = '$defpasien' ORDER BY tgl DESC LIMIT 1");
																$data_penyakit = mysqli_fetch_array($query_penyakit);
																$id_penyakit = $data_penyakit['id'] ?? null;
															}
														?>
														<tr>
															<td><?php echo $i; ?></td>
															<th><?php echo ucwords($row['nama_ruang']); ?></th>
															<td><?php echo ($row['tgl_masuk'] == "") ? 'Belum digunakan' : tgl_indo($row['tgl_masuk']); ?></td>
															<td>
																<?php
																if ($defpasien == '') {
															...
															<td>
																<?php
																if ($row["status"] == "0") {
																	echo '<div class="badge badge-pill badge-success mb-1"><i class="ion-checkmark-round"></i> Tersedia</div>';
																} elseif ($row["status"] == "1") {
																	echo '<div class="badge badge-pill badge-danger mb-1"><i class="ion-close"></i> Dipakai</div>';
																} else {
																	echo '<div class="badge badge-pill badge-warning mb-1"><i class="ion-gear-b"></i>  Dalam Perbaikan</div>';
																}
																?>
															</td>
															...
															<td>
																<?php if ($row['status'] == '1') { ?>
																	...
																	<!-- Tombol Pasien Keluar dengan ID Penyakit -->
																	<a class="btn btn-warning btn-action mr-1" data-toggle="tooltip" title="Pasien keluar"
																		data-confirm="Pasien Keluar|Apakah benar pasien yang bernama <b><?php echo ucwords($namapasien["nama_pasien"]) ?></b> akan keluar?"
																		data-confirm-yes="window.location.href = 'updateriwayat.php?id=<?php echo $defpasien; ?>&id_penyakit=<?php echo $id_penyakit; ?>'">
																...
																<?php } else { ?>
																	...
																	<a class="btn btn-danger btn-action mr-1" data-toggle="tooltip" title="Hapus" data-confirm="Hapus Data|Apakah anda ingin menghapus data ini?" data-confirm-yes="window.location.href = 'auth/delete_admin.php?type=ruang_inap&id=<?php echo $row['id']; ?>'">
